...
class ConfigDung : WindowScreen(ElementaVersion.V1) {
    ...
    override fun onDrawScreen(matrixStack: UMatrixStack, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        Fonts.dinofansSmall.drawSmoothString("MAIN", mainCons.getX()+(mainButton!!.getWidth()/2- Fonts.dinofansSmall.getStringWidth("MAIN")/2).toFloat(), 1+mainCons.getY()+(mainButton!!.getHeight()/2- Fonts.dinofansSmall.height/2).toFloat(), Color.white.rgb)
        if(!mainHovering)mainButton!!.setColor(Color(30,30,30))
        Fonts.dinofans.drawSmoothCenteredString("DUNGEONS", dungCons.getX()+(dungButton!!.getWidth()/2), 1+dungCons.getY()+(dungButton!!.getHeight()/2- Fonts.dinofansSmall.height/2).toFloat(), Color.cyan.rgb)
        if(!dungHovering) dungButton!!.setColor(Color(65,65,65))
    }
}